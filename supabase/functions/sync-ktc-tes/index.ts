...
async function scrapeKTCTEs(format: string = 'dynasty-superflex'): Promise<ScrapeResult> {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const scrapeResult = await scrapeKTCTEs(format);
    ...
    if (!scrapeResult.ok) {
      return new Response(
        JSON.stringify({
          ok: false,
          error: 'Failed to scrape TE data',
